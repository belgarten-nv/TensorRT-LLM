...
def qkv_head_bounds(
    ...
    kv_head_start = calc_shard(num_key_value_heads, tp_size, tp_rank)
    kv_head_end = calc_shard(num_key_value_heads, tp_size, tp_rank + 1)
...
def shard_dim0_range(tensor: torch.Tensor, start: int, end: int) -> torch.Tensor:
    """Shard a tensor along dim 0 using explicit feature bounds."""
    return tensor[start:end].contiguous()


def shard_dim1_range(tensor: torch.Tensor, start: int, end: int) -> torch.Tensor:
    """Shard a tensor along dim 1 using explicit feature bounds."""
    return tensor[:, start:end].contiguous()
...
def shard_fused_qkv_by_heads(
    ...
    q_start, q_end, kv_start, kv_end = qkv_head_bounds(
        tp_rank, tp_size, num_attention_heads, num_key_value_heads, head_dim
    )
...
def shard_kv_dim0(
    ...
    _, _, kv_start, kv_end = qkv_head_bounds(
        tp_rank, tp_size, num_key_value_heads, num_key_value_heads, head_dim
    )
...
def shard_q_dim0(
    ...
    q_start, q_end, _, _ = qkv_head_bounds(
        tp_rank, tp_size, num_attention_heads, num_attention_heads, head_dim
    )
...
def copy_tp_parameter(
    ...
    if tp_param.ndim >= 2 and tp_param.shape[1] == ref_param.shape[1]:
        if "qkv_proj" in tp_name or "add_qkv_proj" in tp_name:
            ...
            tp_param.data.copy_(
                shard_q_dim0(ref_param.data, tp_rank, tp_size, num_attention_heads, head_dim)
            )
        elif (
            "to_k" in tp_name
            or "to_v" in tp_name
            ...
            tp_param.data.copy_(
                shard_kv_dim0(ref_param.data, tp_rank, tp_size, num_key_value_heads, head_dim)
            )
        ...
        if "to_add_out" in tp_name:
            tp_param.data.copy_(
                shard_kv_dim1(ref_param.data, tp_rank, tp_size, num_key_value_heads, head_dim)
            )
        elif "to_out" in tp_name:
            tp_param.data.copy_(
                shard_q_dim1(ref_param.data, tp_rank, tp_size, num_attention_heads, head_dim)
            )
        else:
